...
class gz_decode_message(Node):
    def __init__(self):
        ...
        self.rudder_publisher = self.create_publisher(Float64,'model/rudder_joint',10) 
        self.rudder_gz_msg = Float64()

        self.sail_publisher = self.create_publisher(Float64,'model/main_sail_joint',10)
        ...
        self.airdir_subscriber = self.create_subscription(JointState,'wind_angle',self.gz_windpos,10)
        self.airdir_subscriber

        self.airdir_message=JointState()
        self.airdir_subscriber = self.create_subscription(JointState,'wind_speed',self.gz_windpos,10)
        self.airdir_subscriber
    ...
    def rudder_command(self,msg):
        self.rudder_gz_msg.data=msg.rudder_angle
        self.rudder_publisher.publish(self.rudder_gz_msg)
        
    def sail_command(self,msg):
        self.sail_gz_msg.data=msg.sail_angle
    ...
    def gz_air(self,msg):
        self.windsensor_message.wind_speed=msg.fluid_pressure
        self.windsensor_message.wind_angle=self.airdir
        self.windsensor_publisher.publish(self.windsensor_message)
    def gz_windpos(self,msg):
        self.airdir=msg.position*180/np.pi
